...
export default function CoinTrackerPage() {
  ...
  const AlertCard = ({ item }: { item: WatchItem }) => {
    ...
    const diffMsg = p !== undefined
      ? isPct
        ? `الحالي: ${c!==undefined?(c>=0?'+':'')+c.toFixed(2)+'%':'...'}`
        : `الحالي: $${fmt(p)}`
      : 'جاري التحقق...'
      ...
      <div style={{background:'var(--surface)',border:`1px solid ${trg?'rgba(245,200,66,0.4)':'var(--border)'}`,borderRadius:'12px',padding:'16px',transition:'border-color 0.2s'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'12px'}}>
          <div>
            ...
            <div style={{fontSize:'12px',color:'var(--muted)'}}>{ALERT_LABELS[item.alertType]} <span style={{color:'var(--text)',fontFamily:'var(--mono)',fontWeight:700}}>{isPct?item.alertValue+'%':'$'+fmt(item.alertValue)}</span></div>
          </div>
          <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
            {trg&&<span style={{padding:'3px 8px',borderRadius:'5px',fontSize:'11px',fontWeight:700,background:'rgba(245,200,66,0.12)',color:'var(--yellow)',border:'1px solid rgba(245,200,66,0.3)'}}>🔔 تنبّه!</span>}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
          <div style={{background:'var(--surface-2)',borderRadius:'8px',padding:'10px'}}>
            <div style={{fontSize:'10px',color:'var(--muted)',marginBottom:'3px'}}>السعر الحالي</div>
            <div style={{fontFamily:'var(--mono)',fontWeight:700,fontSize:'13px'}}>{p!==undefined?'$'+fmt(p):'...'}</div>
          </div>
          <div style={{background:'var(--surface-2)',borderRadius:'8px',padding:'10px'}}>
            <div style={{fontSize:'10px',color:'var(--muted)',marginBottom:'3px'}}>التغير 24h</div>
        ...
        {trg&&(
          <div style={{marginTop:'10px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span style={{fontSize:'11px',color:'var(--muted)'}}>{item.triggeredAt?'تنبّه: '+new Date(item.triggeredAt).toLocaleString('ar-SA'):''}</span>
            <button onClick={()=>resetTriggered(item.id)} style={{background:'transparent',border:'1px solid var(--border)',color:'var(--cyan)',padding:'4px 10px',borderRadius:'6px',fontSize:'11px',cursor:'pointer',fontFamily:'inherit',fontWeight:700}}>إعادة تعيين</button>
          </div>
        )}
      </div>
    )
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',color:'var(--text)',padding:'24px'}}>
      <div style={{maxWidth:'1000px',margin:'0 auto'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'24px',flexWrap:'wrap',gap:'12px'}}>
          <div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
              <p style={{color:'var(--muted)',fontSize:'13px'}}>تنبيهات سعرية لحظية</p>
              {lastRefresh&&<span style={{fontSize:'11px',color:'var(--dim)',display:'flex',alignItems:'center',gap:'4px'}}><span className="live-dot"/>آخر تحديث: {lastRefresh.toLocaleTimeString('ar-SA')}</span>}
            </div>
          </div>
          <button onClick={()=>setShowForm(true)} style={{background:'var(--cyan)',color:'#000',border:'none',padding:'9px 20px',borderRadius:'8px',fontWeight:700,fontSize:'13px',cursor:'pointer',fontFamily:'inherit'}}>+ إضافة تنبيه</button>
        </div>

        {/* Stats */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'24px'}}>
          {[
            { label:'تنبيهات نشطة', value:active.length, color:'var(--cyan)' },
            { label:'تنبيهات مُشغَّلة', value:triggered.length, color:'var(--yellow)' },
            { label:'الإجمالي', value:items.length, color:'var(--text)' },
          ].map((s,i)=>(
        ...
        <div style={{display:'flex',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'9px',padding:'3px',gap:'2px',width:'fit-content',marginBottom:'16px'}}>
          {([['active','النشطة',active.length],['triggered','المُشغَّلة',triggered.length]] as const).map(([id,label,cnt])=>(
        ...
        {(tab==='active'?active:triggered).length===0
          ?<div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'12px',padding:'48px',textAlign:'center',color:'var(--muted)'}}>
            ...
            {tab==='active'?'لا توجد تنبيهات نشطة — اضغط "+ إضافة تنبيه"':'لم يُشغَّل أي تنبيه بعد'}
          </div>
          ...
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:9998,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}} onClick={e=>{if(e.target===e.currentTarget)setShowForm(false)}}>
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'16px',padding:'28px',width:'100%',maxWidth:'400px',boxShadow:'0 24px 64px rgba(0,0,0,0.4)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
                <h2 style={{fontSize:'18px',fontWeight:900}}>إضافة تنبيه</h2>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
                <div>
                  <label style={{fontSize:'12px',fontWeight:600,color:'var(--muted)',display:'block',marginBottom:'6px'}}>العملة</label>
                ...
                <div>
                  <label style={{fontSize:'12px',fontWeight:600,color:'var(--muted)',display:'block',marginBottom:'6px'}}>نوع التنبيه</label>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'6px'}}>
                    {(Object.entries(ALERT_LABELS) as [AlertType,string][]).map(([v,l])=>(
                ...
                <div>
                  <label style={{fontSize:'12px',fontWeight:600,color:'var(--muted)',display:'block',marginBottom:'6px'}}>
                    القيمة {form.alertType.includes('change')?'(%)':'(USD)'}
                  </label>
